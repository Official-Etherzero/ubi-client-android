package com.ubiRobot.modules.walletsetting;
...
import com.ubiRobot.modules.walletsetting.BackupMnemonic.MnemonicBackupActivity;
import com.ubiRobot.modules.walletsetting.exportKeystore.ExportKeystoreActivity;
import com.ubiRobot.modules.walletsetting.updatePassword.UpdatePasswordActivity;
...
//        setWalletIcon.setImageResource(iconResourceId);
        ...
        if (balances.containsKey("UBI")) {
            String cryptoBalance = CurrencyUtils.getFormattedAmount(this, "UBI", new BigDecimal(Util.isNullOrEmpty(balances.get("UBI")) ? "0" : balances.get("UBI")));
            setWalletName.setText(cryptoBalance);
        } else {
            setWalletName.setText("0 UBI");
        }
                                ...
                                if (TextUtils.equals(wallet.getPassword(), Md5Utils.md5(pwd))) {
                                    ...
                                    if (WalletUtils.deleteWallet(wallet.getId())) {
                                        if (Util.isNullOrEmpty(SharedPrefsUitls.getInstance().getCurrentWallet())) {
                                            SharedPrefsUitls.getInstance().putUserId("");
                                            SharedPrefsUitls.getInstance().putUBIAddress("");
